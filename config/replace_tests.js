            ...
            if (testFilePath.includes("*")) {
                ...
                glob(testFilePath, options, function (er, currentFilePaths) {
                    for (const path of currentFilePaths) {
                        ...
                        fs.readdir(`./${TESTS_FOLDER}/99x-Inventory-master-template-final-hackathon-javascript/tests`, (err, originalFiles) => {
...
const copyFile = (srcFileName, destPath) => {
    fs.copyFile(`${TESTS_FOLDER}/99x-Inventory-master-template-final-hackathon-javascript/tests/${srcFileName}`, `../${destPath}`, (err) => {
        if (err) throw err;
    });
    return "Copied";
}
